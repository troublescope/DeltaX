...
def stats(update, context):
    ...
    if config_dict["SHOW_LIMITS_IN_STATS"]:
        ...
        torrent_direct = (
            "No Limit Set"
            if TORRENT_DIRECT_LIMIT == ""
            else f"{TORRENT_DIRECT_LIMIT}GB/Link"
        )
        ...
        zip_unzip = (
            "No Limit Set" if ZIP_UNZIP_LIMIT == "" else f"{ZIP_UNZIP_LIMIT}GB/Link"
        )
        total_task = (
            "No Limit Set"
            if TOTAL_TASKS_LIMIT == ""
            else f"{TOTAL_TASKS_LIMIT} Total Tasks/Time"
        )
        user_task = (
            "No Limit Set"
            if USER_TASKS_LIMIT == ""
...
help_user = telegraph.create_page(
    title=f"{config_dict['TITLE_NAME']} Help", content=help_string_telegraph_user
)["path"]
...
help_admin = telegraph.create_page(
    title=f"{config_dict['TITLE_NAME']} Help", content=help_string_telegraph_admin
)["path"]
...
if config_dict["SET_BOT_COMMANDS"]:
    botcmds = [
        ...
        (
            f"{BotCommands.QbZipMirrorCommand[0]}",
            "Mirror torrent and upload as zip using qb",
        ),
        (
            f"{BotCommands.QbUnzipMirrorCommand[0]}",
        ...
        (
            f"{BotCommands.QbZipLeechCommand[0]}",
...
def main():
    ...
    if config_dict["WALLCRAFT_CATEGORY"]:
        for page in range(1, 20):
            r2 = rget(
                f"https://wallpaperscraft.com/catalog/{config_dict['WALLCRAFT_CATEGORY']}/1280x720/page{page}"
            )
            soup2 = BeautifulSoup(r2.text, "html.parser")
            x = soup2.select(
                'img[src^="https://images.wallpaperscraft.com/image/single"]'
            )
            for img in x:
    ...
    if config_dict["WALLTIP_SEARCH"]:
        for page in range(1, 3):
            r2 = rget(
                f"https://www.wallpapertip.com/s/{config_dict['WALLTIP_SEARCH']}/{page}"
            )
            soup2 = BeautifulSoup(r2.text, "html.parser")
    ...
    if config_dict["WALLFLARE_SEARCH"]:
        try:
            for page in range(1, 20):
                r2 = rget(
                    f"https://www.wallpaperflare.com/search?wallpaper={config_dict['WALLFLARE_SEARCH']}&width=1280&height=720&page={page}"
                )
                soup2 = BeautifulSoup(r2.text, "html.parser")
                x = soup2.select(
    ...
    if config_dict["PIXABAY_API_KEY"]:
        try:
            ...
            if config_dict["PIXABAY_CATEGORY"]:
                PIXABAY_ENDPOINT += f"&category={config_dict['PIXABAY_CATEGORY']}"
            if config_dict["PIXABAY_SEARCH"]:
                PIXABAY_ENDPOINT += f"&q={q(config_dict['PIXABAY_SEARCH'])}"
