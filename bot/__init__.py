...
if DATABASE_URL:
    ...
    db = conn.mltb
    if config_dict := db.settings.config.find_one(
        {"_id": bot_id}
...
try:
    ...
    if not premium_session:
        LOGGER.error(
            "Cannot initialized User Session. Please regenerate USER_SESSION_STRING"
        )
    else:
        ...
        if (premium_session.get_me()).is_premium:
            if not LEECH_LOG:
                ...
                try:
                    premium_session.send_message(
                        OWNER_ID, "You must set LEECH_LOG for uploads, Exiting Now..."
                    )
            ...
            premium_session.stop()
            LOGGER.info(
                f"Not using rss. if you want to use fill RSS_CHAT_ID and DATABASE_URL variables."
            )
except:
    USER_SESSION_STRING = ""
    premium_session = ""
...
MULTI_WORKING_PROGRESS_STR = (
    MULTI_WORKING_PROGRESS_STR.replace("'", "")
    .replace('"', "")
    .replace("[", "")
    .replace("]", "")
    .replace(",", "")
