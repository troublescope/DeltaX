...
class DbManger:
    ...
    def db_load(self):
        ...
        if self.__db.settings.aria2c.find_one({"_id": bot_id}) is None:
            self.__db.settings.aria2c.update_one(
        ...
        if self.__db.settings.qbittorrent.find_one({"_id": bot_id}) is None:
            self.__db.settings.qbittorrent.update_one(
        ...
        if self.__db.users.find_one():
            rows = self.__db.users.find(
                {}
            )  # return a dict ==> {_id, is_sudo, is_auth, as_doc, thumb}
            for row in rows:
                uid = row["_id"]
                del row["_id"]
    ...
    def update_private_file(self, path):
        ...
        self.__db.settings.files.update_one(
    ...
    def update_userval(self, user_id, data, value=None):
        ...
        if value is not None:
            ...
            dbval = False
        self.__db.users.update_one(
            {"_id": user_id}, {"$set": {data: dbval}}, upsert=True
        )
        self.__conn.close()
    ...
    def rss_update(self, title):
        if self.__err:
            return
        self.__db.rss[bot_id].update_one(
            {"_id": title}, {"$set": rss_dict[title]}, upsert=True
        )
        self.__conn.close()
