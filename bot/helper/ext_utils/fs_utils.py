...
def split_file(
    ...
    leech_split_size = int(
        (user_dict and user_dict.get("split_size")) or config_dict["TG_SPLIT_SIZE"]
    )
    parts = ceil(size / leech_split_size)
    if (
        (user_dict and user_dict.get("equal_splits")) or config_dict["EQUAL_SPLITS"]
    ) and not inLoop:
    ...
    if get_media_streams(path)[0]:
        ...
        while i <= parts:
            ...
            if listener.suproc.returncode == -9:
                ...
                LOGGER.warning(
                    f"Retrying without map, -map 0 not working in all situations. Path: {path}"
                )
                try:
                    osremove(out_path)
                except:
                ...
                LOGGER.warning(
                    f"Unable to split this video, if it's size less than {config_dict['TG_SPLIT_SIZE']} will be uploaded as it is. Path: {path}"
                )
                try:
                    osremove(out_path)
                except:
            ...
            if lpd == 0:
                ...
                if not noMap:
                    ...
                    LOGGER.warning(
                        f"This file has been splitted with default stream and audio, so you will only see one part with less size from orginal one because it doesn't have all streams and audios. This happens mostly with MKV videos. noMap={noMap}. Path: {path}"
                    )
                    break
            elif lpd <= 4:
                osremove(out_path)
