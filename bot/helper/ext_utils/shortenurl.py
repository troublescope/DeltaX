...
def short_url(longurl, user_id):
    ...
    try:
        ...
        try:
            ...
            longurl = cget(
                "http://tinyurl.com/api-create.php", params=dict(url=longurl)
            ).text
        if "shorte.st" in SHORTENER:
            disable_warnings()
            return cget(
                f"http://api.shorte.st/stxt/{SHORTENER_API}/{longurl}", verify=False
            ).text
            ...
            response = (
                create_scraper()
                .post(shorten_url, json={"long_url": longurl}, headers=headers)
                .json()
            )
            ...
            disable_warnings()
            return cget(
                f"http://ouo.io/api/{SHORTENER_API}?s={longurl}", verify=False
            ).text
        elif "adfoc.us" in SHORTENER:
            disable_warnings()
            return cget(
                f"http://adfoc.us/api/?key={SHORTENER_API}&url={longurl}", verify=False
            ).text
            ...
            disable_warnings()
            return cget(
                f"https://shrinkme.io/api?api={SHORTENER_API}&url={quote(longurl)}&format=text"
            ).text
        else:
            return cget(
                f"https://{SHORTENER}/api?api={SHORTENER_API}&url={quote(longurl)}&format=text"
            ).text
