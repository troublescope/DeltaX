...
from bot import TIME_GAP_STORE, config_dict
...
def timegap_check(message):
    if message.from_user.id in TIME_GAP_STORE:
        if int(time() - TIME_GAP_STORE[message.from_user.id]) < config_dict["TIME_GAP"]:
            wtime = timeformatter(
                (
                    int(TIME_GAP_STORE[message.from_user.id])
                    + config_dict["TIME_GAP"]
                    - int(time())
                )
                * 1000
            )
            ...
            message.reply_text(text=text, parse_mode="markdown", quote=True)
            return True
        else:
