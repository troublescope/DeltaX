...
def __onDownloadStarted(api, gid):
    ...
    try:
        ...
        if any(
            ...
            IS_USRTD = (
                user_data[user_id].get("is_usertd")
                ...
                if sname is not None:
                    ...
                    if smsg:
                        ...
                        if html:
                            ...
                            return sendMessage(
                                smsg, listener.bot, listener.message, button
                            )
...
def add_aria2c_download(link: str, path, listener, filename, auth, ratio, seed_time):
    ...
    if is_magnet(link):
        ...
        links, error = indexScrape(
            {"page_token": "", "page_index": 0}, link, auth, folder_mode=True
        )
        if error:
