...
def add_qb_torrent(link, path, listener, ratio, seed_time):
    ...
    try:
        ...
        if ext_hash is None or len(ext_hash) < 30:
            sendMessage(
                "Not a torrent! Qbittorrent only for torrents!",
                listener.bot,
                listener.message,
            )
            return
        ...
        if len(tor_info) > 0:
            ...
            return
        if link.startswith("magnet:"):
            op = client.torrents_add(
                link, save_path=path, ratio_limit=ratio, seeding_time_limit=seed_time
            )
        else:
            op = client.torrents_add(
                torrent_files=[link],
        ...
        if op.lower() == "ok.":
            ...
            sendMessage(
                "This is an unsupported/invalid link.", listener.bot, listener.message
            )
            __remove_torrent(client, ext_hash)
            return
        tor_info = tor_info[0]
...
def __onSeedFinish(client, tor):
    ...
    try:
        ...
        listener.onUploadError(
            f"Seeding stopped with Ratio: {round(tor.ratio, 3)} and Time: {get_readable_time(tor.seeding_time)}"
        )
    except:
        pass
    __remove_torrent(client, tor.hash)
    ...
    try:
        ...
        if not listener.select and not listener.isLeech and IS_USRTD == False:
            ...
            if qbname is not None:
                qbmsg, button = GoogleDriveHelper(user_id=user_id).drive_list(
                    qbname, True
                )
                if qbmsg:
                    __onDownloadError(
                        "File/Folder is already available in Drive.", client, tor
                    )
...
def __check_limits(client, tor):
    ...
    if (
        any([ZIP_UNZIP_LIMIT, LEECH_LIMIT, TORRENT_DIRECT_LIMIT, STORAGE_THRESHOLD])
    ...
    if (
        ...
        and not is_paid(user_id)
        and (
            size >= (DAILY_MIRROR_LIMIT - getdailytasks(user_id, check_mirror=True))
            or DAILY_MIRROR_LIMIT <= getdailytasks(user_id, check_mirror=True)
        )
    ):
    ...
    if (
        ...
        and not is_paid(user_id)
        and (
            size >= (DAILY_LEECH_LIMIT - getdailytasks(user_id, check_leech=True))
            or DAILY_LEECH_LIMIT <= getdailytasks(user_id, check_leech=True)
        )
    ):
            ...
            for tor_info in client.torrents_info():
                if tor_info.state == "metaDL":
                    ...
                    if (
                        ...
                        Thread(
                            target=__onDownloadError,
                            args=("Dead Torrent!", client, tor_info),
                        ).start()
                    else:
                    ...
                    if (
                        ...
                        and tor_info.hash not in STOP_DUP_CHECK
                    ):
                        STOP_DUP_CHECK.add(tor_info.hash)
                        __stop_duplicate(client, tor_info)
                    if (
                        config_dict["TORRENT_DIRECT_LIMIT"]
                        or config_dict["ZIP_UNZIP_LIMIT"]
                        or config_dict["LEECH_LIMIT"]
                        or config_dict["STORAGE_THRESHOLD"]
                        or config_dict["DAILY_MIRROR_LIMIT"]
                        or config_dict["DAILY_LEECH_LIMIT"]
                    ) and tor_info.hash not in LIMITS_CHECK:
                        LIMITS_CHECK.add(tor_info.hash)
                        __check_limits(client, tor_info)
                elif tor_info.state == "stalledDL":
                    TORRENT_TIMEOUT = config_dict["TORRENT_TIMEOUT"]
                    if (
                        tor_info.hash not in RECHECKED
                        and 0.99989999999999999 < tor_info.progress < 1
                    ):
                        msg = f"Force recheck - Name: {tor_info.name} Hash: "
                        msg += (
                            f"{tor_info.hash} Downloaded Bytes: {tor_info.downloaded} "
                        )
                        msg += (
                            f"Size: {tor_info.size} Total Size: {tor_info.total_size}"
                        )
                        LOGGER.error(msg)
                        ...
                        TORRENT_TIMEOUT
                        and time() - STALLED_TIME.get(tor_info.hash, 0)
                        >= TORRENT_TIMEOUT
                    ):
                        Thread(
                            target=__onDownloadError,
                    ...
                    and tor_info.hash not in UPLOADED
                    and tor_info.state
                    ...
                    tor_info.state in ["pausedUP", "pausedDL"]
                    and tor_info.hash in SEEDING
                ):
