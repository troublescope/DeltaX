        ...
        if not self.obj.is_playlist:
            ...
            if match:
                ...
                with download_dict_lock:
                    download_dict[self.obj.listener.uid] = ConvertStatus(
                        self.obj.name, self.obj.size, self.obj.gid, self.obj.listener
                    )
...
class YoutubeDLHelper:
    ...
    def __onDownloadStart(self, from_queue):
        with download_dict_lock:
            download_dict[self.listener.uid] = YtDlpDownloadStatus(
                self, self.listener, self.gid
            )
        if not from_queue:
            ...
            with YoutubeDL(self.opts) as ydl:
                try:
                    ...
                    if self.is_playlist and (
                        not path.exists(dpath) or len(listdir(dpath)) == 0
                    ):
    ...
    def add_download(self, link, dpath, name, qual, playlist, args, from_queue=False):
        ...
        if (
            ...
            if sname:
                smsg, button = GoogleDriveHelper(user_id=user_id).drive_list(
                    name, True, True
                )
                if smsg:
                    self.__onDownloadError(
        ...
        if (
            not limit_exceeded
            and (MAX_PLAYLIST := config_dict["MAX_PLAYLIST"])
        ...
        if (
            not limit_exceeded
            and (YTDLP_LIMIT := config_dict["YTDLP_LIMIT"])
            and user_id != OWNER_ID
            and not is_sudo(user_id)
            and not is_paid(user_id)
        ):
            ...
            if self.__size > limit:
                ...
                limit_exceeded += (
                    f'Your {"Playlist" if self.is_playlist else "Video"} size\n'
                )
                limit_exceeded += f"is {get_readable_file_size(self.__size)}"
        if (
            not limit_exceeded
            and (LEECH_LIMIT := config_dict["LEECH_LIMIT"])
            and self.listener.isLeech
            and user_id != OWNER_ID
            and not is_sudo(user_id)
            and not is_paid(user_id)
        ):
        ...
        if (
            ...
            and (
                self.__size
                >= (DAILY_MIRROR_LIMIT - getdailytasks(user_id, check_mirror=True))
        ...
        elif not self.listener.isLeech:
            msize = getdailytasks(user_id, upmirror=self.__size, check_mirror=True)
            LOGGER.info(
        ...
        if (
            ...
            and (
                self.__size
                >= (DAILY_LEECH_LIMIT - getdailytasks(user_id, check_leech=True))
            ...
            if config_dict["PAID_SERVICE"] is True:
                mssg += f"\n#Buy Paid Service"
            return self.__onDownloadError(mssg)
        elif self.listener.isLeech:
            lsize = getdailytasks(user_id, upleech=self.__size, check_leech=True)
            LOGGER.info(
        ...
        if self.is_playlist:
            self.opts[
                "outtmpl"
            ] = f"{dpath}/{self.name}/%(title,fulltitle,alt_title)s%(season_number& |)s%(season_number&S|)s%(season_number|)02d%(episode_number&E|)s%(episode_number|)02d%(height& |)s%(height|)s%(height&p|)s%(fps|)s%(fps&fps|)s%(tbr& |)s%(tbr|)d.%(ext)s"
        elif not args:
            ...
            if added_to_queue:
                ...
                with download_dict_lock:
                    download_dict[self.listener.uid] = QueueStatus(
                        self.name, self.__size, self.gid, self.listener, "Dl"
                    )
                self.listener.onDownloadStart()
                sendStatusMessage(self.listener.message, self.listener.bot)
                return
