...
from bot import LOGGER, bot, config_dict, dispatcher, ulist_listener, user_data
...
def list_buttons(update, context):
    ...
    if len(context.args) == 0:
        return sendMessage(
            "Send a search key along with command", context.bot, update.message
        )
    isRecur = False
...
def select_type(update, context):
    ...
    item_type = data[1]
    editMessage(
        f"<b>Searching for <i>{listener_info[1]}</i></b>\n\n<b>Type</b>: {item_type.capitalize()} | <b>Recursive </b>: {listener_info[2]}",
        message,
    )
    Thread(
        target=_list_drive,
        args=(listener_info, message, item_type, context.bot, listener_info[0]),
    ).start()
...
def _list_drive(listener, bmsg, item_type, bot, user_id):
    ...
    if msg:
        if (user_dict and user_dict.get("ulist_typ") == "HTML") or (
            not user_dict and config_dict["LIST_MODE"].lower() == "html"
        ):
...
╰ <b>CC :</b> <a href='tg://user?id={user_id}'>{bot.get_chat(user_id).first_name}</a>\n"""
        ...
        buttons.sbutton("Close", f"cari {user_id} clo", "footer")
        editMessage(
            "Choose the Page no. from below :", query.message, buttons.build_menu(7)
        )
    else:
        try:
