...
from bot import LOGGER, config_dict, dispatcher
...
def wayback(update, context):
    ...
    try:
        ...
        return sendMessage("Not a valid link for wayback.", context.bot, update)
    sent = sendMessage(
        "Running WayBack. Wait about 20 secs.", context.bot, update.message
    )
    retLink = saveWebPage(link)
    if not retLink:
        return editMessage("Cannot archieved. Try again later.", sent)
    editMessage(
        f"Saved webpage: {short_url(retLink, update.message.from_user.id)}", sent
    )
