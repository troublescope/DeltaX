...
def _mirror_leech(
    bot, message, isZip=False, extract=False, isQbit=False, isLeech=False
):
    buttons = ButtonMaker()
    ...
    if (
        ...
        and config_dict["DAILY_TASK_LIMIT"] <= getdailytasks(user_id)
    ):
    ...
    if user_id != OWNER_ID and not is_sudo(user_id) and not is_paid(user_id):
        if config_dict["PAID_SERVICE"] is True:
            ...
            if USER_TASKS_LIMIT == get_user_task(user_id):
                return sendMessage(
                    f"<b>Bot Total Task Limit : {USER_TASKS_LIMIT} \nYour Tasks : {get_user_task(user_id)}\n#User_limit_exceed</b>\n#Buy Paid Service",
                    bot,
                    message,
                )
        else:
            ...
            if file_ is None:
                ...
                if multi > 1:
                    ...
                    Thread(
                        ...
                        args=(bot, nextmsg, isZip, extract, isQbit, isLeech),
                    ).start()
                return
            else:
    ...
    if multi > 1:
        ...
        sleep(4)
        Thread(
            target=_mirror_leech, args=(bot, nextmsg, isZip, extract, isQbit, isLeech)
        ).start()
...
def start_ml(extra, s_listener):
    ...
    if is_gdrive_link(link):
        ...
        Thread(
            target=add_mega_download,
            args=(link, f"{DOWNLOAD_DIR}{listener.uid}/", listener, name),
        ).start()
    elif isQbit and (is_magnet(link) or ospath.exists(link)):
        Thread(
            target=add_qb_torrent,
            args=(link, f"{DOWNLOAD_DIR}{listener.uid}", listener, ratio, seed_time),
        ).start()
    else:
        mesg = message.text.split("\n")
        if len(mesg) > 1:
...
authfilter = (
    CustomFilters.authorized_chat
    if config_dict["QB_MIRROR_ENABLED"] is True
    else CustomFilters.owner_filter
)
...
leech_handler = CommandHandler(
    BotCommands.LeechCommand, leech, filters=authfilter | CustomFilters.authorized_user
)
