...
def anilist(update, context: CallbackContext, aniid=None, u_id=None):
    if not aniid:
        ...
        if len(squery) == 1:
            sendMessage(
                "<i>Provide AniList ID / Anime Name / MyAnimeList ID</i>",
                context.bot,
                update.message,
            )
            return
        ...
        vars = {"id": aniid}
    animeResp = (
        rpost(url, json={"query": ANIME_GRAPHQL_QUERY, "variables": vars})
        .json()["data"]
        .get("Media", None)
    )
    if animeResp:
        ...
        if aniid:
            ...
            try:
                msg.reply_photo(
                    ...
                    reply_markup=btns.build_menu(3),
                )
            except:
                msg.reply_photo(
...
def setAnimeButtons(update, context):
    ...
    message.edit_caption(
        caption=msg, parse_mode=ParseMode.HTML, reply_markup=btns.build_menu(1)
    )
...
def character(update, context, aniid=None, u_id=None):
    ...
    if not aniid:
        ...
        user_id = int(u_id)
    json = (
        rpost(url, json={"query": character_query, "variables": vars})
        .json()["data"]
        .get("Character", None)
    )
    if json:
        ...
        if aniid:
            ...
            if img:
                update.effective_message.reply_photo(
                    photo=img, caption=msg, reply_markup=rlp_mk
                )
            else:
...
def setCharacButtons(update, context):
    ...
    if user_id != int(data[1]):
        ...
        if len(sptext) > 900:
            sptext = f"{sptext[:900]}..."
        message.edit_caption(
            caption=f"<b>Spoiler Ahead :</b>\n\n<tg-spoiler>{markdown(sptext).replace('<p>', '').replace('</p>', '')}</tg-spoiler>",
            parse_mode=ParseMode.HTML,
            reply_markup=btns.build_menu(1),
        )
    elif data[2] == "home":
        query.answer()
...
def manga(update, context):
    ...
    if json:
        ...
        if image:
            ...
            update.effective_message.reply_text(
                msg, parse_mode=ParseMode.MARKDOWN, reply_markup=buttons.build_menu(1)
            )
...
anifilters = (
    CustomFilters.authorized_chat if ANILIST_ENABLED else CustomFilters.owner_filter
)
ANIME_HANDLER = CommandHandler(
    "anime", anilist, filters=anifilters | CustomFilters.authorized_user
)
CHARACTER_HANDLER = CommandHandler(
    "character", character, filters=anifilters | CustomFilters.authorized_user
)
MANGA_HANDLER = CommandHandler(
    "manga", manga, filters=anifilters | CustomFilters.authorized_user
)
WEEBHELP_HANDLER = CommandHandler(
    "weebhelp", weebhelp, filters=anifilters | CustomFilters.authorized_user
)
