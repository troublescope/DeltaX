...
from bot import (DATABASE_URL, DOWNLOAD_DIR, LOGGER, Interval, aria2, bot,
                 config_dict, download_dict, download_dict_lock, non_queued_dl,
                 non_queued_up, queue_dict_lock, queued_dl, queued_up,
                 status_reply_dict_lock, tgBotMaxFileSize, user_data)
from bot.helper.ext_utils.bot_utils import (change_filename, get_bot_pm,
                                            get_readable_file_size,
                                            get_readable_time,
                                            getGDriveUploadUtils, is_magnet,
                                            is_url)
...
from bot.helper.mirror_utils.status_utils.tg_upload_status import \
    TgUploadStatus
...
from bot.helper.telegram_helper.message_utils import (
    auto_delete_upload_message, delete_all_messages, sendMessage, sendPhoto,
    update_all_messages)
...
class MirrorLeechListener:
    ...
    def onDownloadComplete(self):
        ...
        if self.isZip:
            ...
            TG_SPLIT_SIZE = int(
                (user_dict and user_dict.get("split_size"))
                or config_dict["TG_SPLIT_SIZE"]
            )
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ, name):
        ...
        up_path, name, _ = change_filename(
            ...
            user_id_,
            all_edit=False,
            mirror_type=(False if self.isLeech else True),
        )
        ...
        if "link_logs" in user_data:
            try:
                ...
                for link_log in user_data["link_logs"]:
                    bot.sendMessage(
                        link_log,
                        text=slmsg + upper + source_link + lower,
            ...
            if reply_to is not None:
                try:
                    ...
                    pass
        AUTO_DELETE_UPLOAD_MESSAGE_DURATION = config_dict[
        ...
        if self.isLeech:
            ...
            if not files:
                if config_dict["PICS"]:
                    ...
                    uploadmsg = sendMessage(
                        msg, self.bot, self.message, buttons.build_menu(2)
                    )
            else:
                fmsg = ""
                for index, (link, name) in enumerate(files.items(), start=1):
                    ...
                    if len(fmsg.encode() + msg.encode()) > 2000:
                        ...
                        if not BOT_PM_X:
                            if config_dict["PICS"]:
                                uploadmsg = sendPhoto(
                                    msg + fmsg + pmwarn + logleechwarn + warnmsg,
                                    self.bot,
                                    self.message,
                                    choice(config_dict["PICS"]),
                                    buttons.build_menu(2),
                            ...
                            Thread(
                                ...
                                args=(bot, self.message, uploadmsg),
                            ).start()
                        fmsg = ""
                if fmsg != "":
                    sleep(1.5)
            ...
            if (
                config_dict["BUTTON_FOUR_NAME"] != ""
                and config_dict["BUTTON_FOUR_URL"] != ""
            ):
                buttons.buildbutton(
                    f"{config_dict['BUTTON_FOUR_NAME']}",
                    f"{config_dict['BUTTON_FOUR_URL']}",
                )
            if (
                config_dict["BUTTON_FIVE_NAME"] != ""
                and config_dict["BUTTON_FIVE_URL"] != ""
            ):
                buttons.buildbutton(
                    f"{config_dict['BUTTON_FIVE_NAME']}",
                    f"{config_dict['BUTTON_FIVE_URL']}",
                )
            if (
                config_dict["BUTTON_SIX_NAME"] != ""
                and config_dict["BUTTON_SIX_URL"] != ""
            ):
                buttons.buildbutton(
                    f"{config_dict['BUTTON_SIX_NAME']}",
                    f"{config_dict['BUTTON_SIX_URL']}",
                )
        ...
        if BOT_PM_X and self.message.chat.type != "private":
            ...
            if (
                ...
                and config_dict["AUTO_DELETE_UPLOAD_MESSAGE_DURATION"] == -1
            ):
                reply_to.delete()
    ...
    def onDownloadError(self, error):
        try:
            if (
                config_dict["AUTO_DELETE_UPLOAD_MESSAGE_DURATION"] != -1
