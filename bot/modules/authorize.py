...
def addPaid(update, context):
    ...
    if id_:
        if (
            is_paid(id_)
            and ex_date
            and user_data[id_].get("expiry_date")
            and (ex_date == user_data[id_].get("expiry_date"))
        ):
...
addsudo_handler = CommandHandler(
    BotCommands.AddSudoCommand, addSudo, filters=CustomFilters.owner_filter
)
removesudo_handler = CommandHandler(
    BotCommands.RmSudoCommand, removeSudo, filters=CustomFilters.owner_filter
)
addpaid_handler = CommandHandler(
    BotCommands.AddPaidCommand, addPaid, filters=CustomFilters.owner_filter
)
removepaid_handler = CommandHandler(
    BotCommands.RmPaidCommand, removePaid, filters=CustomFilters.owner_filter
)
