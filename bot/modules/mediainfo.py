...
from bot import LOGGER, app, config_dict, dispatcher
...
def mediainfo(update, context):
    ...
    if process.stderr.decode():
        return LOGGER.error("mediainfo not installed. Read readme.")
    # mediainfo control -
    ...
    if not file:
        return sendMessage(help_msg, context.bot, update.message)
    sent = sendMessage(
        "Running mediainfo. Downloading your file.", context.bot, update.message
    )
    try:
        VtPath = os.path.join("Mediainfo", str(message.from_user.id))
        if not os.path.exists("Mediainfo"):
            os.makedirs("Mediainfo")
    ...
    editMessage(
        short_url(f"https://telegra.ph/{help}", update.message.from_user.id), sent
    )
...
mediainfo_handler = CommandHandler(
