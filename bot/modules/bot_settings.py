...
default_values = {
    ...
    "START_BTN1_NAME": "Main Group",
    "START_BTN1_URL": "https://t.me/deltamltb",
    "START_BTN2_NAME": "Main Channel",
    "START_BTN2_URL": "https://t.me/kai_verse",
    "AUTHOR_NAME": "Tommy",
    "AUTHOR_URL": "https://t.me/troublescoope",
    "TITLE_NAME": "DeltaX",
    "GD_INFO": "DeltaX uploader.",
    "CREDIT_NAME": "troublescoope",
    "NAME_FONT": "code",
    "CAPTION_FONT": "code",
    "FINISHED_PROGRESS_STR": "●",
    "UN_FINISHED_PROGRESS_STR": "○",
    "MULTI_WORKING_PROGRESS_STR": "○◔◗◒◕◖●".split(" "),
    "IMAGE_URL": "https://telegra.ph/file/c452fda633799933ea019.png",
...
def load_config():
    ...
    EXTENSION_FILTER = environ.get("EXTENSION_FILTER", "html")
    ...
    AUTO_DELETE_UPLOAD_MESSAGE_DURATION = environ.get(
    ...
    TORRENT_DIRECT_LIMIT = (
        "" if len(TORRENT_DIRECT_LIMIT) == 0 else float(TORRENT_DIRECT_LIMIT)
    )
    ...
    if (
        len(FINISHED_PROGRESS_STR) == 0
        or len(FINISHED_PROGRESS_STR) == 0
        or len(MULTI_WORKING_PROGRESS_STR) == 0
    ):
        FINISHED_PROGRESS_STR = "█"  # '■'
        UN_FINISHED_PROGRESS_STR = "▒"  # '□'
        MULTI_WORKING_PROGRESS_STR = "▁ ▂ ▃ ▄ ▅ ▆ ▇".split(" ")
...
def get_buttons(key=None, edit_type=None):
    ...
    if key is None:
        ...
        buttons.sbutton("Close", "botset close")
        msg = (
            "Send private file: config.env, token.pickle, accounts.zip, list_drives.txt, cookies.txt, categories.txt or .netrc."
    ...
    if value.lower() == "true":
        ...
        for download in downloads:
            if not download.is_complete:
                try:
                    aria2.client.change_option(
                        download.gid, {"bt-stop-timeout": f"{value}"}
                    )
                except Exception as e:
                    LOGGER.error(e)
...
def edit_bot_settings(update, context):
    ...
    if not CustomFilters.owner_query(user_id):
        query.answer(
            text="You don't have premision to use these buttons!", show_alert=True
        )
    elif data[1] == "close":
        query.answer()
        ...
        if data[2] in [
            ...
            "TELEGRAM_API",
            "AUTHORIZED_CHATS",
            "RSS_DELAY" "DATABASE_URL",
            "BOT_TOKEN",
            "DOWNLOAD_DIR",
        ]:
            query.answer(
                text="Restart required for this edit to take effect!", show_alert=True
            )
        else:
            query.answer()
        if handler_dict.get(message.chat.id):
        ...
        value_handler = MessageHandler(
            filters=Filters.text
            & Filters.chat(message.chat.id)
            & (CustomFilters.owner_filter | CustomFilters.sudo_user),
            callback=partial_fnc,
        )
        dispatcher.add_handler(value_handler)
        while handler_dict[message.chat.id]:
...
bot_settings_handler = CommandHandler(
