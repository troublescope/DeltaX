...
def category_change(update, context):
    ...
    if len(context.args) == 1:
        ...
        if not dl:
            sendMessage(
                "This is not an active task make sure you reply to an active task!",
                context.bot,
                update.message,
            )
            return
    ...
    if dl.status() not in [
        ...
        sendMessage(
            f"Task should be on {MirrorStatus.STATUS_DOWNLOADING} or {MirrorStatus.STATUS_PAUSED} or {MirrorStatus.STATUS_WAITING}",
            context.bot,
            update.message,
        )
        return
    ...
    if (
        listener
        and (len(CATEGORY_NAMES) > 1 or len(getUserTDs(user_id)[0]) > 1)
        ...
        sendMessage(
            "Cannot change the category for this task!", context.bot, update.message
        )
...
def confirm_category(update, context):
    ...
    if user_id != listener.message.from_user.id and not CustomFilters.owner_query(
        user_id
    ):
        ...
        if listener.u_index == u_index:
            return query.answer(
                f"{getUserTDs(listener.user_id)[0][u_index]} is selected already!",
                show_alert=True,
            )
        query.answer()
        listener.c_index = c_index
...
change_category_handler = CommandHandler(
    BotCommands.SelectCategory,
    category_change,
