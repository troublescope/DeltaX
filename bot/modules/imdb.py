...
def imdb_search(update: Update, context: CallbackContext):
    if " " in update.message.text:
        ...
        editMessage(
            "<b><i>Here What I found on IMDb.com</i></b>", k, buttons.build_menu(1)
        )
    else:
        sendMessage(
            "<i>Send Movie / TV Series Name along with /imdb Command</i>",
            context.bot,
            update.message,
        )
...
imdbfilters = (
    CustomFilters.authorized_chat if IMDB_ENABLED else CustomFilters.owner_filter
)
IMDB_HANDLER = CommandHandler(
    "imdb", imdb_search, filters=imdbfilters | CustomFilters.authorized_user
)
