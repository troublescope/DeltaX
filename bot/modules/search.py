...
def initiate_search_tools():
    ...
    if SEARCH_API_LINK := config_dict["SEARCH_API_LINK"]:
        ...
        try:
            ...
            SITES = {
                str(site): str(site).capitalize() for site in res["supported_sites"]
            }
            SITES["all"] = "All"
...
def torser(update, context):
    ...
    if SITES is None and not SEARCH_PLUGINS:
        ...
        sendMessage(
            "Choose site to search | Plugins:", context.bot, update.message, button
        )
...
def __getResult(search_results, key, message, method):
    ...
    path = [
        telegraph.create_page(title=f"{config_dict['TITLE_NAME']}", content=content)[
            "path"
        ]
