...
example_dict = {
    ...
    + config_dict["SA_MAIL"]
    + " to your TD as Content Manager</b>",
...
def get_user_settings(from_user, key=None):
    ...
    if key is None:
        ...
        imdb = (
            user_dict["imdb_temp"]
            if user_dict and user_dict.get("imdb_temp")
            else "Not Exists"
        )
        anilist = (
            user_dict["ani_temp"]
            if user_dict and user_dict.get("ani_temp")
            else "Not Exists"
        )
        ytq = (
            user_dict["yt_ql"]
            if user_dict and user_dict.get("yt_ql")
            else config_dict["YT_DLP_QUALITY"]
            if config_dict["YT_DLP_QUALITY"]
            else "Not Exists"
        )
        ulist = (
            user_dict["ulist_typ"]
            if user_dict and user_dict.get("ulist_typ")
            else f'{config_dict["LIST_MODE"].lower().capitalize()} (Default)'
        )
        dailytl = (
        ...
        dailytas = (
            ...
            and not is_sudo(user_id)
            and not is_paid(user_id)
            and config_dict["DAILY_TASK_LIMIT"]
            else config_dict.get("DAILY_TASK_LIMIT", "Unlimited")
        ...
        if user_dict and user_dict.get("dly_tasks"):
            ...
            lastused = "Bot Not Used"

        buttxt = (
            "Change/Delete YT-DLP Quality"
            if ytq != "Not Exists"
            else "Set YT-DLP Quality"
        )
        buttons.sbutton(buttxt, f"userset {user_id} suniversal yt_ql universal")
        buttxt = (
            "Change/Delete List Type"
            if ulist != f'{config_dict["LIST_MODE"].lower().capitalize()} (Default)'
            else "Set List Type"
        )
        ...
        if imdb != "Not Exists":
            ...
            buttons.sbutton(
                "Change/Delete IMDB",
                f"userset {user_id} suniversal imdb_temp universal",
            )
            buttons.sbutton("Show IMDB Template", f"userset {user_id} showimdb")
        else:
            buttons.sbutton(
                "Set IMDB", f"userset {user_id} suniversal imdb_temp universal"
            )
        if anilist != "Not Exists":
            anival = "Exists"
            buttons.sbutton(
                "Change/Delete AniList",
                f"userset {user_id} suniversal ani_temp universal",
            )
            buttons.sbutton("Show AniList Template", f"userset {user_id} showanilist")
        else:
            buttons.sbutton(
                "Set AniList", f"userset {user_id} suniversal ani_temp universal"
            )
...
"""
        ...
        prefix = (
            user_dict["mprefix"]
            if user_dict and user_dict.get("mprefix")
            else "Not Exists"
        )
        suffix = (
            user_dict["msuffix"]
            if user_dict and user_dict.get("msuffix")
            else "Not Exists"
        )
        remname = (
            user_dict["mremname"]
            if user_dict and user_dict.get("mremname")
            else "Not Exists"
        )
        ...
        dailyup = (
            get_readable_file_size(getdailytasks(user_id, check_mirror=True))
            if config_dict["DAILY_MIRROR_LIMIT"]
            and user_id != OWNER_ID
            and not is_sudo(user_id)
            and not is_paid(user_id)
        ...
        if usertd != "Not Exists":
            ...
            if config_dict["ENABLE_USR_TD"]:
                buttons.sbutton(
                    "Change/Delete User TD(s)",
                    f"userset {user_id} suniversal usertd mirror",
                )
            else:
                buttons.sbutton(
                    "Change/Delete User TD(s)", f"userset {user_id} usertdxdisable"
                )
            buttons.sbutton("Show User TD(s)", f"userset {user_id} showusertds")
        else:
            if config_dict["ENABLE_USR_TD"]:
                buttons.sbutton(
                    "Set User TD(s)", f"userset {user_id} suniversal usertd mirror"
                )
            else:
...
"""
        ...
        prefix = (
            user_dict["prefix"]
            if user_dict and user_dict.get("prefix")
            else "Not Exists"
        )
        suffix = (
            user_dict["suffix"]
            if user_dict and user_dict.get("suffix")
            else "Not Exists"
        )
        caption = (
            user_dict["caption"]
            if user_dict and user_dict.get("caption")
            else "Not Exists"
        )
        remname = (
            user_dict["remname"]
            if user_dict and user_dict.get("remname")
            else "Not Exists"
        )
        cfont = (
            user_dict["cfont"][0]
            if user_dict and user_dict.get("cfont")
            else "<b>Not Exists</b>"
        )
        userlog = (
            user_dict["userlog"]
            if user_dict and user_dict.get("userlog")
            else "Not Exists"
        )
        dailytlle = (
        ...
        dailyll = (
            get_readable_file_size(getdailytasks(user_id, check_leech=True))
            if config_dict["DAILY_LEECH_LIMIT"]
            and user_id != OWNER_ID
            and not is_sudo(user_id)
            and not is_paid(user_id)
        ...
        lsplit = (
            get_readable_file_size(user_dict["split_size"])
            if user_dict and user_dict.get("split_size")
            else get_readable_file_size(config_dict["TG_SPLIT_SIZE"]) + "(Default)"
        )
        ...
        if ospath.exists(thumbpath):
            thumbmsg = "Exists"
            buttons.sbutton(
                "Change/Delete Thumbnail", f"userset {user_id} sthumb leech"
            )
        ...
        esplits = (
            ...
            or user_dict
            and user_dict.get("equal_splits")
            else "Disabled"
        )
        ...
        buttxt = (
            "Change/Delete Leech Split"
            if lsplit
            != get_readable_file_size(config_dict["TG_SPLIT_SIZE"]) + "(Default)"
            else "Set Leech Split"
        )
        buttons.sbutton(buttxt, f"userset {user_id} suniversal split_size leech")
        if cfont != "<b>Not Exists</b>":
            buttons.sbutton("Remove CapFont", f"userset {user_id} cfont")
...
def edit_user_settings(update, context):
    ...
    if user_id != int(data[1]):
        ...
        update_user_ldata(
            user_id,
            "equal_splits",
            not bool(user_dict and user_dict.get("equal_splits")),
        )
        update_user_settings(message, query.from_user, "leech")
        if DATABASE_URL:
            DbManger().update_user_data(user_id)
        ...
        query.answer(
            text="Now, Your Files Will Be Mirrored/Cloned ON Your Personal TD!",
            show_alert=True,
        )
        ...
        query.answer(
            text="Now, Your Files Will Be Mirrorred/Cloned ON Global TD!",
            show_alert=True,
        )
        update_user_settings(message, query.from_user, "mirror")
        if DATABASE_URL:
        ...
        query.answer(
            text="Now, Your Files will not be send to your PM anymore!", show_alert=True
        )
        update_user_settings(message, query.from_user, "universal")
        if DATABASE_URL:
            DbManger().update_user_data(user_id)
        ...
        if ospath.lexists(path):
            ...
            delo = sendPhoto(
                text=msg, bot=context.bot, message=message, photo=open(path, "rb")
            )
            Thread(args=(context.bot, update.message, delo)).start()
        else:
            query.answer(text="Send new settings command.")
        ...
        if (
            ...
            query.answer(
                "You not Not Paid User to Use this Feature. \n#Buy Paid Service",
                show_alert=True,
            )
            return
        query.answer()
        ...
        if data[3] == "split_size":
            ...
            buttons.sbutton(
                "Set Custom Font Style", f"userset {user_id} font leech", "header"
            )
        ...
        FONT_SPELL = {
            ...
            "s": "<s>Strike</s>",
            "u": "<u>Underline</u>",
            "tg-spoiler": "<tg-spoiler>Spoiler</tg-spoiler>",
        }
        buttons = ButtonMaker()
        ...
        if user_dict and user_dict.get("cfont"):
            cf = user_data[user_id]["cfont"]
        else:
            cf = [f'{FONT_SPELL[config_dict["CAPTION_FONT"]]} (Default)']
        editMessage(
            "<u>Change your Font Style from below:</u>\n\n• Current Style : " + cf[0],
            message,
            btns,
        )
    elif data[2] == "Spoiler":
        ...
        buttons.sbutton("Close", f"userset {user_id} close", "footer")
        if user_id in user_data and user_data[user_id].get("ulist_typ"):
            ul = user_data[user_id]["ulist_typ"]
        else:
            ul = f'{config_dict["LIST_MODE"].lower().capitalize()} (Default)'
        editMessage(
            "<u>Change your List Fetch Mode from below:</u>\n\n• Current Mode : " + ul,
            message,
            buttons.build_menu(2),
        )
    elif data[2] == "ulist":
        ...
        if user_dict and user_dict.get("usertd"):
            ...
            try:
                ...
                query.answer(
                    "UserTD details send in Private (PM) Successfully", show_alert=True
                )
            except:
                query.answer(
                    "Start the Bot in Private and Try Again to get your UserTD Details!",
                    show_alert=True,
                )
    else:
        query.answer()
        ...
        try:
            for ud, dd in d.items():
                if ud == "is_auth" and dd is True:
                    auth_chat += (
                        f"<b>{bot.get_chat(u).title}</b> ( <code>{u}</code> )\n"
                    )
            ...
            if u == "is_leech_log":
                leechlogs = (
                    "\n".join(
                        f"<b>{bot.get_chat(ll).title}</b> ( <code>{ll}</code> )"
                        for ll in d
                    )
                    + "\n"
                )
...
pdetails_handler = CommandHandler(
    command=BotCommands.PaidUsersCommand,
    callback=sendPaidDetails,
    filters=CustomFilters.owner_filter | CustomFilters.sudo_user,
)
users_settings_handler = CommandHandler(
    BotCommands.UsersCommand,
    send_users_settings,
    filters=CustomFilters.owner_filter | CustomFilters.sudo_user,
)
user_set_handler = CommandHandler(
